...
router.post("/delete",helper.authenticateToken, async (req, res) => {
    try {
        if (req.token.userid && mongoose.Types.ObjectId.isValid(req.token.userid)) {
            ...
            const deletedUser = await primary.model(constants.MODELS.customers, customers)
                .findByIdAndDelete(req.body.id);
            if (deletedUser) {
                return responseManager.onSuccess('customer deleted successfully.', 1, res);
            } else {
                return responseManager.onError('customer not found.', res);
            }
        } else {
            return responseManager.badrequest('Invalid token to delete customer, please try again.', res);
        }
    } catch (error) {
        return responseManager.onError(error, res);
    }
...
router.post('/createcustomer', helper.authenticateToken, async (req, res) => {
    try {
      ...
      // Find user data
      const userData = await primary.model(constants.MODELS.customers, customers)
        .findOne({ name: name });
      if (userData) {
        return responseManager.onSuccess('Customer already exists...!',0, res);
    }else{
        await primary.model(constants.MODELS.customers, customers).create({name:name});
  ...
  router.post("/customerData",helper.authenticateToken, async (req, res) => {
    try {
        if (req.token.userid && mongoose.Types.ObjectId.isValid(req.token.userid)) {
            let { name }= req.body
            const primary = mongoConnection.useDb(constants.DEFAULT_DB);
            const customerData = await primary.model(constants.MODELS.expenses, expenses)
                .find({customer:name});
            if (customerData) { 
